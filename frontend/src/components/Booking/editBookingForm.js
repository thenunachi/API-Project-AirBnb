...
const errors=[];
if(edate < sdate){
  errors.push("Checkout date can not come before check in date");
}
...
<input type="date" id="start" name="trip-start" min={new Date().toISOString().split('T')[0]}
  value={sdate}
  // min={new Date()}
  onChange={(e) => setSDate(e.target.value)} />
      ...
      <div className="dates-picked">
        ...
        <input type="date" name="trip-start" min={new Date().toISOString().split('T')[0]}
          value={edate}
